    ...
    def handle_staff(staff_id):
        try:
            ...
            if request.method == 'DELETE':
                # Get all active staff members except the one being deleted
                other_staff = User.query.filter(
                    User.role == 'staff',
                    User.is_active == True,
                    User.id != staff_id
                ).all()
                ...
                if not other_staff:
                    return jsonify({
                        'error': 'Cannot delete the staff member',
                        'message': 'No other active staff members available to reassign leads'
                    }), 400
                ...
                try:
                    ...
                    # Get all leads assigned to the staff member
                    leads = Lead.query.filter_by(assigned_staff_id=staff_id).all()
                    lead_count = len(leads)

                    if lead_count > 0:
                        # Calculate leads per staff member (round up)
                        leads_per_staff = -(-lead_count // len(other_staff))  # Ceiling division
                        
                        # Distribute leads among other staff members
                        for i, lead in enumerate(leads):
                            # Assign to staff member using round-robin
                            new_staff = other_staff[i % len(other_staff)]
                            lead.assigned_staff_id = new_staff.id
                            
                            # Add a followup note about the reassignment
                            followup = Followup(
                                lead_id=lead.id,
                                staff_id=new_staff.id,
                                followup_date=datetime.datetime.utcnow(),
                                notes=f"Lead reassigned from {staff.name} due to staff member deletion"
                            )
                            db.session.add(followup)

                    # Update all followups to be owned by the admin
                    Followup.query.filter_by(staff_id=staff_id).update({
                        'notes': Followup.notes + f" (Originally by: {staff.name})",
                        'staff_id': current_user.id  # Assign to admin
                    })

                    # Update all sales records to be owned by the admin
                    Sale.query.filter_by(staff_id=staff_id).update({
                        'staff_id': current_user.id  # Assign to admin
                    })
                    ...
                    return jsonify({
                        ...
                        'leads_reassigned': lead_count,
                        'reassigned_to': [{'id': s.id, 'name': s.name} for s in other_staff]
                    }), 200

                except Exception as e:
                    db.session.rollback()
                    logger.error(f"Error in staff deletion transaction: {str(e)}")
                    return jsonify({
...
# Screen Streaming Namespace
@socketio.on('start_stream', namespace='/screen')
@jwt_required()
def start_stream(data):
    try:
        current_user = get_current_user()
        if not current_user or current_user.role != 'staff':
            return emit('error', {'message': 'Unauthorized'}, room=request.sid)

        admin_room = 'admin_room'
        join_room(admin_room)
        emit('stream_started', {'message': 'Streaming started'}, room=admin_room)
    except Exception as e:
        app.logger.error(f"Error in start_stream: {str(e)}")
        emit('error', {'message': 'Failed to start stream'}, room=request.sid)

@socketio.on('stream_data', namespace='/screen')
def stream_data(data):
    try:
        admin_room = 'admin_room'
        emit('screen_data', data, room=admin_room)
    except Exception as e:
        app.logger.error(f"Error in stream_data: {str(e)}")

@socketio.on('stop_stream', namespace='/screen')
@jwt_required()
def stop_stream():
    try:
        current_user = get_current_user()
        ...
        emit('stream_stopped', {'message': 'Streaming stopped'}, room=admin_room)
    except Exception as e:
        app.logger.error(f"Error in stop_stream: {str(e)}")
        emit('error', {'message': 'Failed to stop stream'}, room=request.sid)

# Admin Endpoint to View Streams
@app.route('/admin/streams', methods=['GET'])
@jwt_required()
def get_active_streams():
    try:
        ...
        # Return active streams (this can be enhanced with a database or in-memory tracking)
        return jsonify({'message': 'Active streams available'}), 200
    except Exception as e:
        app.logger.error(f"Error in get_active_streams: {str(e)}")
        return jsonify({'error': str(e)}), 500
